...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_tip'])) {
    ...
    if (!empty($text)) {
        $stmt = $conn->prepare("UPDATE tips SET text = ? WHERE id = ?");
...
$stmt = $conn->prepare("SELECT text FROM tips WHERE id = ?");
            ...
            <div class="form-container">
                ...
                <form method="post" class="form-grid">
                    <div class="form-group">
                        ...
                        <textarea id="text" name="text" rows="4" required><?php echo htmlspecialchars($tip['text']); ?></textarea>
                    </div>
                    <button type="submit" name="update_tip">Update Tip</button>
                </form>
                <a href="tips.php" class="back-link">&larr; Back to All Tips</a>
            </div>
        </main>
    </div>
</body>
